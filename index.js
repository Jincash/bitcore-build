 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`botcore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`botcore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'botcore-' + name : 'botcore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/botcore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + fullname + ' --external botcore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = browserifyPath + ' --require ./index.js:botcore-lib -o botcore-lib.js';
    }
  ...
  var release = function(importance, cb) {
    ...
    return runsequence(
      ...
      // Run git push botcoin-core $VERSION
      'release:push-tag',
      // Run npm publish
      'release:publish',
